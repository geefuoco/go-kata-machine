...
func TestServedBuildings(t *testing.T){
  ...
  assert.Equal(t, 3, util.ServedBuildings(buildings, routerLocations, routerRanges))

  buildings2 := []int{1, 2, 2, 1, 2, 1, 1, 2, 2, 5, 6, 2}
  routerLocations2 := []int{1, 6, 9}
  routerRanges2 := []int{1, 6, 2}
  assert.Equal(t, 7, util.ServedBuildings(buildings2, routerLocations2, routerRanges2))
}
